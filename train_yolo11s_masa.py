...
MODEL_CFG = Path("ultralytics/cfg/models/11/yolo11s-masa-p4.yaml")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a YOLO11s MaSA variant.")
    parser.add_argument("--data", default=str(DATA_YAML), help="Dataset yaml path.")
    parser.add_argument(
        "--model",
        default=str(MODEL_CFG),
        help="Model yaml or checkpoint path. Use the MaSA yaml for fresh training, or a checkpoint for resume.",
    )
    parser.add_argument(
        "--weights",
    ...
    parser.add_argument("--name", default="yolo11s_masa", help="Experiment name.")

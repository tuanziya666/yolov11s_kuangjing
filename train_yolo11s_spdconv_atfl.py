...
MODEL_CFG = Path("ultralytics/cfg/models/11/yolo11s-SPDConv.yaml")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a YOLO11s SPDConv variant with ATFL classification loss.")
    parser.add_argument("--data", default=str(DATA_YAML), help="Dataset yaml path.")
    parser.add_argument(
        "--model",
        default=str(MODEL_CFG),
        help="Model yaml or checkpoint path. Use the SPDConv yaml for fresh training, or a checkpoint for resume.",
    )
    parser.add_argument(
        "--weights",
    ...
    parser.add_argument("--name", default="yolo11s_spdconv_atfl", help="Experiment name.")

    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if not WCIoU or confidence is None:
                    return ciou  # CIoU

                confidence = confidence.clamp(eps, 1.0)
                ciou_loss = 1.0 - ciou
                w_occlusion = torch.exp(-confidence)
                w_confidence = (1.0 - confidence).pow(gamma)
                adaptive_conf_loss = -torch.log(confidence) * w_confidence
                total_loss = loss_lambda * (ciou_loss * w_occlusion) + (1.0 - loss_lambda) * adaptive_conf_loss
...
def ap_per_class(
    ...
    """
    # Keep validation metric computation in float32 to reduce peak host-memory usage.
    ...
    x, prec_values = np.linspace(0, 1, 1000, dtype=np.float32), []

    # Average precision, precision and recall curves
    ap = np.zeros((nc, tp.shape[1]), dtype=np.float32)
    ...
    for ci, c in enumerate(unique_classes):
        ...
        fpc = np.cumsum(1.0 - tp[i], axis=0, dtype=np.float32)
        tpc = np.cumsum(tp[i], axis=0, dtype=np.float32)

        # Recall
        recall = tpc / (np.float32(n_l) + eps)  # recall curve
        r_curve[ci] = np.interp(-x, -conf[i], recall[:, 0], left=0)  # negative x, xp because xp decreases

        # Precision
        precision = tpc / np.maximum(tpc + fpc, eps)  # precision curve
    ...
    prec_values = np.array(prec_values, dtype=np.float32)  # (nc, 1000)
